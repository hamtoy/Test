...
class TestIntegratedQAPipelineInit:
    ...
    def test_init_loads_environment(self) -> None:
        ...
        with patch("src.qa.pipeline.require_env") as mock_require:
            ...
            with patch("src.qa.pipeline.QAKnowledgeGraph"):
                with patch("src.qa.pipeline.DynamicTemplateGenerator"):
    ...
    def test_init_creates_kg_and_template_gen(self) -> None:
        ...
        with patch("src.qa.pipeline.require_env", return_value="test"):
            with patch("src.qa.pipeline.QAKnowledgeGraph") as mock_kg:
                with patch("src.qa.pipeline.DynamicTemplateGenerator") as mock_gen:
                    pipeline = IntegratedQAPipeline()
            ...
            with patch("src.qa.pipeline.QAKnowledgeGraph"):
                with patch("src.qa.pipeline.DynamicTemplateGenerator") as mock_gen:
                    ...
                    with patch("src.qa.pipeline.find_violations", return_value=[]):
                        ...
                        assert result["valid"] is False
                        assert any(
                            "error_pattern" in v for v in result["violations"]
                        )
...
class TestRunIntegratedPipeline:
    ...
    def test_run_integrated_pipeline_success(self, tmp_path: Path) -> None:
        ...
        meta_file.write_text(
            json.dumps({"image_path": "test.png"}), encoding="utf-8"
        )
