...
class TestCacheConfigConstants:
    ...
    def test_min_tokens_for_caching_value(self) -> None:
        """Verify MIN_TOKENS_FOR_CACHING is 2048."""
        assert CacheConfig.MIN_TOKENS_FOR_CACHING == 2048

    def test_min_tokens_rationale_exists(self) -> None:
...
class TestCacheManagerThreshold:
    ...
    def test_should_cache_below_threshold(self, cache_manager: Any) -> None:
        """경계값 테스트: 2047 토큰."""
        assert cache_manager.should_cache(2047) is False

    def test_should_cache_at_threshold(self, cache_manager: Any) -> None:
        """경계값 테스트: 2048 토큰."""
        assert cache_manager.should_cache(2048) is True

    def test_should_cache_above_threshold(self, cache_manager: Any) -> None:
        """경계값 테스트: 2049 토큰."""
        assert cache_manager.should_cache(2049) is True

    def test_should_cache_short_prompt(self, cache_manager: Any) -> None:
        """실제 사용 케이스: 짧은 프롬프트."""
        assert cache_manager.should_cache(1000) is False

    def test_should_cache_medium_prompt(self, cache_manager: Any) -> None:
        """실제 사용 케이스: 중간 프롬프트."""
        assert cache_manager.should_cache(5000) is True

    def test_should_cache_long_prompt(self, cache_manager: Any) -> None:
        """실제 사용 케이스: 긴 프롬프트."""
...
class TestSavingsEstimation:
    ...
    def test_estimate_savings_small(self, cache_manager: Any) -> None:
        """절감률 추정: 작은 프롬프트."""
        assert cache_manager._estimate_savings(3000) == 50

    def test_estimate_savings_medium(self, cache_manager: Any) -> None:
        """절감률 추정: 중간 프롬프트."""
        assert cache_manager._estimate_savings(10000) == 70

    def test_estimate_savings_large(self, cache_manager: Any) -> None:
        """절감률 추정: 큰 프롬프트."""
