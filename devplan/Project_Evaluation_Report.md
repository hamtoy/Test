...
1. **대형 파일 분리**: 
   - `src/web/api.py` (1695줄) - 복잡도 높음, 모듈 분리 권장
   - `src/agent/core.py` (978줄) - 기능별 분리 필요
   - `src/qa/rag_system.py` (668줄) - 추가 모듈화 필요
   
2. **세션 관리**: 웹 모듈에서 사용자 세션 관리 기능 부족

3. **에러 로깅 상세화**: 일부 모듈에서 예외 처리 시 상세 로그 부족
