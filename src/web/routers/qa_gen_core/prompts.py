...
**2. 본론 (필수)**
- **소제목** 형식 또는 불릿 포인트로 근거 제시
- 소제목: **소제목** (콜론 없이 독립 행)
- 불릿: - **항목명**: 설명 형식

**3. 결론 (필수 - 생략 시 실패)**
- 마지막 문단에서 반드시 종합 결론 작성
- "결론적으로", "따라서", "종합하면" 등으로 시작
- ❌ 결론 문단 없이 끝나면 실패

[올바른 형식 예시]
유럽의 천연가스 가격 급락은 경기 침체 우려를 완화시켜 위험자산 선호 심리를 높이는 요인이었습니다. 이를 바탕으로 천연가스 가격이 다시 급등할 경우, 유럽 투자자들의 심리와 포트폴리오 구성은 정반대의 방향으로 변할 것으로 예상됩니다.

**투자 심리 변화**
천연가스 가격이 다시 급등한다면 유럽의 경기 침체 우려가 재차 부각될 것입니다...

**포트폴리오 구성 변화**
투자 심리가 위축됨에 따라 투자자들은...
- **위험자산 비중 축소**: 주식과 같은 위험자산에 대한 선호도가 낮아질 것입니다.
- **안전자산 비중 확대**: 채권 등 안전자산의 비중을 늘릴 것으로 보입니다.

결론적으로 천연가스 가격의 재급등은 유럽의 경기 침체 우려를 다시 자극하여 투자 심리를 위축시키고, 이는 투자 포트폴리오에서 주식 등 위험자산의 비중을 줄이고 채권과 같은 안전자산의 비중을 늘리는 결과로 이어질 것으로 전망됩니다.

{fewshot_text}

[금지 사항]
- ❌ 첫 줄에 **볼드체 제목** 사용 금지 → 서술형 평문으로
