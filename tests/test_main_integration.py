...
# =============================================================================
# Runpy Tests (Execute __main__ block)
# =============================================================================


class TestRunpyExecution:
    """Tests that execute the __name__ == "__main__" block using runpy."""

    def test_runpy_keyboard_interrupt(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        """Test running main module with KeyboardInterrupt."""
        import runpy
        from src.ui import console
        from src.config.constants import USER_INTERRUPT_MESSAGE
        ...
        with (
            ...
            assert exc_info.value.code == 130
            mock_console_print.assert_called_with(USER_INTERRUPT_MESSAGE)

    def test_runpy_general_exception(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        """Test running main module with general exception."""
        import runpy
        ...
        with (
            ...
            assert exc_info.value.code == 1
            mock_logging_critical.assert_called()

    def test_runpy_success_flow(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        """Test running main module with successful flow."""
        import runpy
        ...
        with (
            ...
            # Verify log_listener.stop was called
            mock_listener.stop.assert_called_once()

    def test_runpy_windows_event_loop_policy(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        """Test Windows event loop policy when running on Windows."""
        import runpy
        ...
        with (
            ...
            # On Windows, policy should be set
