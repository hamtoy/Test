"""단답답변예시.txt를 Neo4j Example 노드로 등록하는 스크립트."""
...
def import_target_short_examples():
    """단답답변예시.txt 파일을 Neo4j에 등록."""
    examples_file = Path("data/examples/단답답변예시.txt")
    ...
    with driver.session() as session:
        for qa in qa_pairs:
            ...
            example_id = (
                f"target_short_{hashlib.sha256(qa_text.encode()).hexdigest()[:16]}"
            )
    ...
    print(f"✅ {saved_count}개 target_short Example 저장 완료!")

    safe_driver.close()


if __name__ == "__main__":
    import_target_short_examples()
