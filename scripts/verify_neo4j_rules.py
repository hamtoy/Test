...
def verify_rules():
    ...
    for qtype in types:
        ...
        # category별 분류
        query_constraints = [c for c in constraints if c.get("category") in ["query", "both"]]
        answer_constraints = [c for c in constraints if c.get("category") in ["answer", "both"]]
        
        print(f"\n🔍 질의 제약사항 ({len(query_constraints)}개):")
        for c in sorted(query_constraints, key=lambda x: x.get("priority", 0), reverse=True):
            print(f"  [{c.get('priority', 0)}] {c.get('name')}")
            print(f"      {c.get('description')[:80]}...")
        
        print(f"\n📝 답변 제약사항 ({len(answer_constraints)}개):")
        for c in sorted(answer_constraints, key=lambda x: x.get("priority", 0), reverse=True):
            print(f"  [{c.get('priority', 0)}] {c.get('name')}")
            print(f"      {c.get('description')[:80]}...")
