...
async def require_agent() -> GeminiAgent:
    ...
    """
    agent = await asyncio.to_thread(ServiceContainer.get_agent)
    if agent is None:
...
async def require_config() -> AppConfig:
    ...
    return await asyncio.to_thread(ServiceContainer.get_config)
...
async def require_knowledge_graph() -> QAKnowledgeGraph:
    ...
    """
    kg = await asyncio.to_thread(ServiceContainer.get_knowledge_graph)
    if kg is None:
...
async def require_pipeline() -> IntegratedQAPipeline:
    ...
    """
    pipeline = await asyncio.to_thread(ServiceContainer.get_pipeline)
    if pipeline is None:
